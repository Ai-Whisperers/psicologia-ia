# Politica de Privacidad

> **DOCUMENTO BORRADOR - REQUIERE REVISION LEGAL**
> **Version:** 1.0
> **Fecha:** Enero 2026
> **Marco Legal:** Ley 1682/2001 (Paraguay) - Proteccion de Datos Personales

---

## 1. Responsable del Tratamiento

**Nombre:** [NOMBRE_PROFESIONAL] [APELLIDO]
**Servicio:** [Nombre del servicio]
**Contacto:** [Email / WhatsApp]
**Ubicacion:** Asuncion, Paraguay

---

## 2. Datos que Recopilamos

### 2.1 Datos de Identificacion
- Nombre completo
- Numero de telefono
- Direccion de correo electronico
- Edad (para verificar mayoria de edad)

### 2.2 Datos del Servicio
- Registros de sesiones (notas escritas)
- Grabaciones de audio/video (solo con consentimiento)
- Historial de comunicaciones (WhatsApp, email)
- Informacion sobre tu bienestar compartida voluntariamente

### 2.3 Datos Tecnicos
- Registros de conexion a videollamadas
- Informacion de pago (no almacenamos datos de tarjetas)

---

## 3. Para Que Usamos Tus Datos

| Finalidad | Base Legal |
|-----------|------------|
| Prestacion del servicio de coaching | Consentimiento |
| Seguimiento de tu progreso | Consentimiento |
| Coordinacion de citas | Interes legitimo |
| Facturacion | Obligacion legal |
| Mejora del servicio | Interes legitimo |

---

## 4. Terceros con Acceso a Tus Datos

### 4.1 Proveedores de Tecnologia

| Proveedor | Servicio | Datos Compartidos |
|-----------|----------|-------------------|
| WhatsApp (Meta) | Comunicacion | Mensajes, audios |
| Google | Videollamadas | Video, audio sesion |
| Calendly | Agendamiento | Nombre, email, horario |

### 4.2 Servicios de IA (Si Aplica)

**IMPORTANTE:** Si consientes el uso de IA:

| Proveedor | Servicio | Datos Compartidos |
|-----------|----------|-------------------|
| OpenAI | Transcripcion (Whisper) | Audio de sesiones |
| Anthropic | Analisis (Claude) | Texto de transcripciones |

Estos proveedores tienen sus propias politicas de privacidad:
- OpenAI: https://openai.com/privacy
- Anthropic: https://anthropic.com/privacy

---

## 5. Donde Se Almacenan Tus Datos

| Tipo de Dato | Ubicacion | Seguridad |
|--------------|-----------|-----------|
| Notas de sesion | Almacenamiento local encriptado | Password + encriptacion |
| Comunicaciones | Servidores de WhatsApp/Google | Encriptacion E2E |
| Backups | Almacenamiento en nube encriptado | Encriptacion AES-256 |

**Nota:** Los datos procesados por APIs de IA viajan a servidores en Estados Unidos durante el procesamiento.

---

## 6. Tiempo de Conservacion

| Tipo de Dato | Periodo de Conservacion |
|--------------|------------------------|
| Datos de identificacion | Mientras dure la relacion + 5 anos |
| Notas de sesion | Mientras dure la relacion + 5 anos |
| Grabaciones de audio | Maximo 30 dias post-procesamiento |
| Datos de facturacion | 10 anos (obligacion fiscal) |

---

## 7. Tus Derechos (Ley 1682/2001)

Tienes derecho a:

### 7.1 Acceso
Solicitar copia de todos los datos que tengo sobre ti.

### 7.2 Rectificacion
Corregir datos inexactos o incompletos.

### 7.3 Supresion
Solicitar la eliminacion de tus datos (con excepciones legales).

### 7.4 Oposicion
Oponerte al tratamiento de tus datos para fines especificos.

### 7.5 Portabilidad
Recibir tus datos en formato estructurado.

**Para ejercer estos derechos:** Contactame a [email] con el asunto "Ejercicio de Derechos ARCO".

---

## 8. Seguridad de los Datos

Implemento las siguientes medidas:

- Dispositivos protegidos con contrasena
- Almacenamiento encriptado
- Autenticacion de dos factores donde sea posible
- Acceso limitado (solo yo accedo a tus datos)
- Backups regulares en ubicacion segura

---

## 9. Transferencias Internacionales

Al usar servicios como WhatsApp, Google, OpenAI o Anthropic, tus datos pueden ser transferidos a servidores fuera de Paraguay, principalmente en Estados Unidos.

Estas transferencias se realizan con proveedores que cumplen con estandares de proteccion de datos reconocidos internacionalmente.

---

## 10. Menores de Edad

Si tienes menos de 18 anos:
- Requerimos consentimiento de tu padre/madre/tutor legal
- El tutor tendra acceso a informacion general del proceso
- Cierta informacion puede mantenerse confidencial segun criterio profesional

---

## 11. Cambios a Esta Politica

Me reservo el derecho de actualizar esta politica. Te notificare de cambios significativos por WhatsApp o email antes de que entren en vigor.

**Ultima actualizacion:** Enero 2026

---

## 12. Contacto

Para preguntas sobre privacidad:

**Email:** [Por definir]
**WhatsApp:** [Por definir]

---

## Notas para el profesional

> **TODO antes de usar este documento:**
> 1. Consultar con abogado sobre cumplimiento Ley 1682
> 2. Verificar si necesitas registrarte como responsable de datos
> 3. Definir procedimiento concreto para solicitudes ARCO
> 4. Confirmar politicas de WhatsApp Business para uso profesional
> 5. Crear version resumida "Aviso de Privacidad" para uso rapido

---

**Este documento es un BORRADOR y requiere revision legal antes de su uso profesional.**
